...
export class LoliconAPI extends plugin {
    ...
    async key_setu(e) {
        ...
        try {
            ...
            if (Array.isArray(result.data) && result.data.length === 0) return e.reply('Σσ(・Д・；)未获取到相关数据', false, { recallMsg: 60 })
            ...
            if (successCount === 0) {
                return e.reply('[LoliconAPI] 获取图片失败！', false, { recallMsg: 60 })
            } else if (failureCount > 0) {
                msgs.push(`[LoliconAPI] 获取图片成功 ${successCount} 张，失败 ${failureCount} 张~`)
            }

            return e.reply(await makeForwardMsg(e, msgs, '( *・ω・)✄╰ひ╯\n割以永治'))
        } catch (err) {
            logger.warn(err)
            return e.reply('Σσ(・Д・；)请检查网络环境', false, { recallMsg: 60 })
        }
    }
    ...
    async set_cd(e) {
        ...
        if (match) {
            ...
            if (/^\d+$/.test(input)) {
                ...
                return e.reply(`[LoliconAPI] 已修改CD为${parseInt(input)}秒！`)
            } else {
    ...
    async set_num(e) {
        ...
        if (match) {
            ...
            if (/^\d+$/.test(input)) {
                ...
                return e.reply(`[LoliconAPI] 已修改限制为${parseInt(input)}张！`)
            } else {
                return e.reply('[WARN] 请输入正确的数字！', true)
            }
        }
    ...
    async setMaster_r18(e) {
        const type = e.msg.replace(/^我(不)?要涩涩$/g, '$1')
        if (type === '不' || type === '') {
            await updateConfig('r18_Master', type === '' ? 1 : 0)
            return e.reply(`真的假的我操？`)
        } else {
            return false
        }
    }
    ...
    async delete_img(e) {
        ...
        return e.reply(`[Success]共清理缓存图片${successCount}张`)
    }
}
